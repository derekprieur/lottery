...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    ...
    const nomOfUserTickets = totalTickets.reduce((total, ticketAddress) => (ticketAddress === address ? total + 1 : total), 0)
    setUserTickets(nomOfUserTickets)
  }, [tickets, address])
  ...
  return (
    <div className='bg-stone-800 min-h-screen flex flex-col'>
      ...
      <Header />
      <Marquee className='bg-stone-800 p-5 mb-5' gradient={false} speed={100}>
      ...
      {winnings > 0 && (
        <div className='max-w-md md:max-w-2xl lg:max-w-4xl mx-auto mt-5'>
          <button onClick={onWithdrawWinnings} className='p-5 bg-gradient-to-b from-emerald-200 to-emerald-800 animate-pulse text-center rounded-xl w-full'>
      ...
      <div className='space-y-5 md:space-y-0 m-5 md:flex md:flex-row items-start justify-center md:space-x-5'>
        <div className='stats-container flex-1 max-w-2xl'>
        ...
        <div className='stats-container space-y-2'>
          <div className='stats-container'>
            ...
            <div className='flex text-white items-center space-x-2 border border-stone-700 p-4'>
              <p>TICKETS</p>
            ...
            <button onClick={handleClick} disabled={expiration?.toString() < Date.now().toString() || remainingTickets?.toNumber() === 0} className='mt-5 w-full bg-gradient-to-br from-emerald-200 to-emerald-800 px-10 py-5 rounded-md text-white shadow-xl disabled:from-gray-600 font-semibold disabled:to-gray-600 disabled:text-gray-100 disabled:cursor-not-allowed'>Buy {quantity} tickets for {ticketPrice && Number(ethers.utils.formatEther(ticketPrice.toString())) * quantity} {currency}</button>
          </div>
          {userTickets > 0 && (
            <div className='stats'>
              <p className='text-lg mb-2'>You have {userTickets} tickets in this draw</p>
              <div className='flex max-w-sm flex-wrap gap-x-2 gap-y-2'>
                {Array(userTickets).fill('').map((_, i) => (
                  <p key={i} className='text-emerald-300 h-20 w-12 bg-emerald-500/30 rounded-lg flex flex-shrink-0 items-center justify-center text-xs italic'>{i + 1}</p>
                ))}
              </div>
            </div>
          )}
